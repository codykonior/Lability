    ...
    process {
        ...
        if ($Path -match '\.meta\.mof$') {
            ...
            foreach ($line in [System.IO.File]::ReadLines($Path)) {
                ...
                $currentLineNumber++;
                if ($line -match '^instance of (?!MSFT_Credential|MSFT_xWebBindingInformation|ServerPermission|SqlPermission)') {
                    ## Ignore MSFT_Credential and MSFT_xWebBindingInformation types.
                    ## Ignore ServerPermission and SqlPermission which are class types in SqlServerDsc.
                    ## There may be other types that need suppressing, but they'll be resource specific
